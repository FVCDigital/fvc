/**
 * Trade FVC card component
 * Allows users to trade USDC for FVC tokens
 * @module components/cards/TradeFVCCard
 */
...
import { calculateFVCAmountFromUSDC } from '@/services/tradingService';
import { BaseCardProps, TradeState } from '@/types';

/**
 * Props for the TradeFVCCard component
 */
interface TradeFVCCardProps extends BaseCardProps {
  /** Additional CSS class names */
  className?: string;
}

/**
 * Trade FVC card component that allows users to trade USDC for FVC tokens
 * @param props - Component props
 * @returns Trade FVC card JSX element
 */
const TradeFVCCard: React.FC<TradeFVCCardProps> = ({ className = '' }) => {
  ...
  const fvcAmount = calculateFVCAmountFromUSDC(usdcAmount, exchangeRate);

  /**
   * Handles the trade transaction
   */
  ...
  if (!address) {
    return (
      <div style={{
        ...
        <div style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Trade FVC</div>
        <div style={{ fontSize: 16, color: theme.secondaryText, textAlign: 'center' }}>
          Connect your wallet to Trade FVC tokens
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
        ...
        >
          {isProcessing ? 'Processing...' : 'Trade FVC'}
        </button>
