...
async function main() {
  console.log("🔧 Fixing FVC Supply to 1 Billion...");
  ...
  const additionalBondingNeeded = TARGET_BONDING_ALLOCATION - currentBondingBalance;
  ...
  console.log("Additional Bonding Needed:", ethers.formatEther(additionalBondingNeeded));
  ...
  try {
    ...
    // Step 2: Transfer exact amount to bonding contract (10M total)
    console.log("\n2. Allocating exactly 10M FVC to bonding contract...");
    
    // First, transfer current bonding balance to owner (to reset)
    if (currentBondingBalance > 0n) {
      console.log("Transferring current bonding balance back to owner...");
      const transferBackTx = await fvc.transfer(owner.address, currentBondingBalance);
      await transferBackTx.wait();
      console.log("✅ Bonding balance transferred back to owner");
    }

    // Now transfer exactly 10M to bonding
    ...
    console.log("Other Allocations:", otherPercentage.toString(), "%");

    if (newTotalSupply === TARGET_TOTAL_SUPPLY && newBondingBalance === TARGET_BONDING_ALLOCATION) {
