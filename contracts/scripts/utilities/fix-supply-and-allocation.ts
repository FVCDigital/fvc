...
async function main() {
  console.log("🔧 Fixing Total Supply to 1B and Bonding to 300M...");
  ...
  try {
    // Step 1: Mint exactly 300M to bonding (if needed)
    const bondingDeficit = TARGET_BONDING_ALLOCATION - currentBondingBalance;
    if (bondingDeficit > 0n) {
      console.log("1. Minting", ethers.formatEther(bondingDeficit), "FVC to bonding...");
      ...
      console.log("1. Bonding already has sufficient allocation");
    }

    // Step 2: Mint exactly 700M to owner (if needed)
    ...
    // Step 3: Burn excess by minting to burn address
    const newTotalSupply = await fvc.totalSupply();
    const excessSupply = newTotalSupply - TARGET_TOTAL_SUPPLY;
    
    if (excessSupply > 0n) {
      console.log("3. Burning excess", ethers.formatEther(excessSupply), "FVC...");
      const burnAddress = "0x000000000000000000000000000000000000dEaD";
      const burnTx = await fvc.mint(burnAddress, excessSupply);
      console.log("Burn transaction hash:", burnTx.hash);
      await burnTx.wait();
      console.log("✅ Excess FVC burned!");
    } else {
      console.log("3. No excess supply to burn");
    }
    ...
    if (finalTotalSupply === TARGET_TOTAL_SUPPLY && finalBondingBalance === TARGET_BONDING_ALLOCATION) {
      ...
      console.log("📝 Total supply:", ethers.formatEther(finalTotalSupply), "FVC (target: 1B)");
